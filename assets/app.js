...
if (hiddenFromSearch && hideFromSearchBtn) {
    ...
    hideFromSearchBtn.querySelector("span").textContent = "Show in Search";
}
...
if (hideFromSearchBtn) {
    hideFromSearchBtn.addEventListener("click", () => {
        ...
        if (hiddenFromSearch) {
            ...
            hideFromSearchBtn.querySelector("span").textContent = "Hide in Search";
...
function createPeer() {
    ...
    p.oniceconnectionstatechange = () => {
        ...
        if (p.iceConnectionState === "disconnected" || p.iceConnectionState === "failed" || p.iceConnectionState === "closed") {
            handlePeerDisconnect();
        }
    };
...
function setupChannel() {
    ...
    dataChannel.onopen = () => updateStatus("Connection established");
...
function handlePeerDisconnect() {
    ...
    // Show system message
    const systemMessage = "The other party closed the connection or the connection was lost.";
...
function startCall(id) {
    ...
    if (connectionStatus) {
        const confirmReconnect = confirm("You are already connected to a chat. Do you want to close the previous chat and establish a new connection?");
...
function sendFile() {
    ...
    try {
        ...
        readNextChunk(); // start the first chunk
